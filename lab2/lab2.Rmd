...
ggplot(ci_adults_children, aes(x = room_type_reserved, y = mean_adults, color = type_of_meal_plan)) +
  ...
  labs(title = "Середня кількість дорослих в залежності від 
       типу номеру та плану харчування", x = "Тип номеру", y = "Середня кількість дорослих") +
  theme_minimal()+
...
ggplot(ci_adults_children, aes(x = room_type_reserved, y = mean_children, color = type_of_meal_plan)) +
  ...
  labs(title = "Середня кількість дітей в залежності від типу номеру та плану харчування", 
       x = "Тип номеру", 
...
ggplot(ci_df, aes(x = Room_Type, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = a, ymax = b), width = 0.2) +
  labs(x = "Тип кімнати", y = "Середня кількість дорослих", title = "В кімнатах 4, 6, 7 в середньому дещо більше дорослих ніж в інших") +
  ylim(0, NA) +
  scale_x_discrete(labels = room_label_vector)+
...
ggplot(ci_df, aes(x = Room_Type, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = a, ymax = b), width = 0.2) +
  labs(x = "Тип кімнати", y = "Середня кількість дітей", title = "В 2 і 7 в середньому більше дітей ніж в інших, в 6 - значно більше") +
  ylim(0, NA) +
  scale_x_discrete(labels = room_label_vector)+
  theme(axis.text.x = element_text(angle = 20, hjust = 0.5), plot.title = element_text(hjust = 0.5))
```
...
ggplot(ci_df, aes(x = Room_Type, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = a, ymax = b), width = 0.2) +
  labs(x = "Тип кімнати", y = "Середня кількість людей", title = "В 6 і 7 типах набагато більше людей ніж в інших") +
  ylim(0, NA) +
  scale_x_discrete(labels = room_label_vector)+
  theme(axis.text.x = element_text(angle = 20, hjust = 0.5), plot.title = element_text(hjust = 0.5))
```
...
ggplot(ci_df, aes(x = Meal_plan, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = a, ymax = b), width = 0.2) +
  labs(x = "План харчування", y = "Середня кількість дорослих", title = "Приблизно однакова кількість для різних планів") +
  ylim(0, NA) +
...
ggplot(ci_df, aes(x = Meal_plan, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = a, ymax = b), width = 0.2) +
  labs(x = "План харчування", y = "Середня кількість дітей", title = "Трохи більше дітей для першого плану") +
  ylim(0, 1)  +
  scale_x_discrete(labels = meal_label_vector) +
  theme(axis.text.x = element_text(angle = , hjust = 0.5), plot.title = element_text(hjust = 0.5))
```
...
ggplot(ci_df, aes(x = Meal_plan, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = a, ymax = b), width = 0.2) +
  labs(x = "План харчування", y = "Середня кількість людей", title = "Інтервали відрізняються несуттєво") +
  ylim(0, NA) +
  scale_x_discrete(labels = meal_label_vector)+
  theme(axis.text.x = element_text(angle = , hjust = 0.5), plot.title = element_text(hjust = 0.5))
```
...
```{r}
booking_status_count <- hotel %>%
  filter(lead_time >= 350) %>%
...
cat("Mean (With requests):", mean_hat_req, "\n")
cat("Mean (Without requests):", mean_hat_no_req, "\n")
cat("T-statistic:", T, "\n")
cat("P-value:", p_value, "\n")
...
# Calculate confidence intervals for 'avg_price_per_room' grouped by 'no_of_nights'
ci_special_requests <- hotel %>%
...
ggplot(ci_special_requests, aes(x = no_of_nights, y = mean_price)) +
  ...
  geom_hline(yintercept = 105, linetype = "dashed", color = "red") +
  labs(title = "~справжнє середнє значення ціни добре аппроксимує 
       середні значення цін по кожному значенню кількості ночей",
       x = "Кількість ночей",
       y = "Середня ціна за номер") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
```{r}
mean(hotel$avg_price_per_room)
```

### ТЕСТ
```{r}
price_range_by_room_type <- hotel %>%
  filter(avg_price_per_room < 540) %>%
  group_by(room_type_reserved) %>%
  summarize(min_price = min(avg_price_per_room, na.rm = TRUE),
            max_price = max(avg_price_per_room, na.rm = TRUE),
            .groups = 'drop') # This will suppress the message
...
# Calculate confidence intervals for 'no_of_nights' grouped by 'room_type_reserved'
ci_room_type_nights <- hotel %>%
  group_by(room_type_reserved) %>%
...
# Print the result (optional)
# print(ci_room_type_nights)

# Plot the result
ggplot(ci_room_type_nights, aes(x = room_type_reserved, y = mean_nights)) +
  geom_point() +
  geom_errorbar(aes(ymin = ci_low_nights, ymax = ci_high_nights), width = 0.2) +
  labs(title = "Confidence Intervals for Average Number of Nights by Room Type",
       x = "Room Type Reserved",
       y = "Mean Number of Nights") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
original_hotel %>% filter(room_type_reserved == 'Room_Type 3')
```
...
ggplot(ci_nights_children, aes(x = as.factor(no_of_adults), y = mean_nights)) +
  geom_point() +
  geom_errorbar(aes(ymin = ci_low_nights, ymax = ci_high_nights), width = 0.2) +
  labs(title = "Середня кількість ночей відносно кількості дорослих", x = "Кількість дорослих", y = "Середня кількість ночей") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
```

```{r}
nrow(hotel %>% filter(repeated_guest != 0))
```
```{r}
nrow(hotel %>% filter(repeated_guest == 0))
```
...
cat("Mean (Required):", mean_hat_req, "\n")
cat("Mean (Not required):", mean_hat_no_req, "\n")
cat("T-statistic:", T, "\n")
cat("P-value:", p_value, "\n")
cat("95% Confidence Interval:", conf.int, "\n")
```
...
cat("Mean (Repeated guest):", mean_hat_req, "\n")
cat("Mean (Not repeated guest):", mean_hat_no_req, "\n")
cat("T-statistic:", T, "\n")
cat("P-value:", p_value, "\n")
cat("95% Confidence Interval:", conf.int, "\n")
